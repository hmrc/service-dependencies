...
class DerivedDeployedDependencyRepositorySpec
  ...
  "put" should {
    "insert new documents" in {
      ...
      repository.find(SlugInfoFlag.QA, Some("group-1"), Some("artifact-1"), None, None).futureValue.sortBy(_.repoName) shouldBe Seq(metaArtefactDependency1, metaArtefactDependency2)
      repository.find(SlugInfoFlag.QA, Some("group-3"), Some("artifact-3"), None, None).futureValue                    shouldBe Seq(metaArtefactDependency3)
    }
  }
  ...
  "find" should {
    "find document in QA & Production" in {
      ...
      repository.put(Seq(metaArtefactDependency1, metaArtefactDependency3)).futureValue
      repository.find(SlugInfoFlag.QA        , group = Some("group-1"), artefact = Some("artifact-1")).futureValue shouldBe Seq(metaArtefactDependency1)
      repository.find(SlugInfoFlag.Production, group = Some("group-1"), artefact = Some("artifact-1")).futureValue shouldBe Seq.empty
      repository.find(SlugInfoFlag.QA        , group = Some("group-3"), artefact = Some("artifact-3")).futureValue shouldBe Seq.empty
      repository.find(SlugInfoFlag.Production, group = Some("group-3"), artefact = Some("artifact-3")).futureValue shouldBe Seq(metaArtefactDependency3)
    }
    ...
    "find document by group, artefact & scope" in {
      ...
      repository.find(SlugInfoFlag.QA, scopes = Some(List(Compile))          , group = Some("group-1"), artefact = Some("artifact-1")).futureValue                    shouldBe Seq(metaArtefactDependency1)
      repository.find(SlugInfoFlag.QA, scopes = Some(List(Provided))         , group = Some("group-1"), artefact = Some("artifact-1")).futureValue                    shouldBe Seq(metaArtefactDependency2)
      repository.find(SlugInfoFlag.QA, scopes = Some(List(Compile, Provided)), group = Some("group-1"), artefact = Some("artifact-1")).futureValue.sortBy(_.repoName) shouldBe Seq(metaArtefactDependency1, metaArtefactDependency2)
    }
    ...
    "find document just by scope" in {
      ...
      repository.put(Seq(metaArtefactDependency1, metaArtefactDependency2)).futureValue
      repository.find(SlugInfoFlag.QA, scopes = Some(List(Compile))          ).futureValue                    shouldBe Seq(metaArtefactDependency1)
      repository.find(SlugInfoFlag.QA, scopes = Some(List(Provided))         ).futureValue                    shouldBe Seq(metaArtefactDependency2)
      repository.find(SlugInfoFlag.QA, scopes = Some(List(Compile, Provided))).futureValue.sortBy(_.repoName) shouldBe Seq(metaArtefactDependency1, metaArtefactDependency2)
    }
  }
}
