...
class BobbyVersionRangeSpec extends AnyWordSpec with Matchers {
  ...
  "A BobbyVersionRange" should {
    ...
    "fail to parse when incomplete version is provided" in {
      BobbyVersionRange.parse("[1.5,)") shouldBe None
    }

    "fail to parse when brackets are missing" in {
      BobbyVersionRange.parse("1.5,)") shouldBe None
      BobbyVersionRange.parse("[1.5,") shouldBe None
      BobbyVersionRange.parse("1.5")   shouldBe None
    }

    "fail to parse when the range is open on both sides" in {
      BobbyVersionRange.parse("(,1.5,)") shouldBe None
    }

    "fail to parse when multiple sets are used" in { // should we support this? Is valid
      BobbyVersionRange.parse("(,1.0],[1.2,)") shouldBe None
    }

    "fail to parse when qualifier is not defined" in {
    ...
    "include 2.3 when the expression is (,2.4]" in {
    ...
    "compare multidigit versions" in {
