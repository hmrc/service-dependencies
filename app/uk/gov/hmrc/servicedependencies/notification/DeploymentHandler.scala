...
, config         = SqsConfig("aws.sqs.deployment", configuration)
  ...
  private def updateDeploymentAndDerivedData(payload: DeploymentHandler.DeploymentEvent, isDeployment: Boolean): EitherT[Future, String, Unit] =
    for {
      oPrevious  <- EitherT.right[String](deploymentRepository.find(payload.environment, slugName = payload.serviceName))
      ...
      _          <- (oPrevious, deployedIn.isEmpty) match {
                      case (Some(prev), true)  => logger.info(s"${prefix(payload)} deleting DERIVED-deployed-dependencies")
                                                  EitherT.right[String](derivedDeployedDependencyRepository.delete(prev.slugName, Some(prev.slugVersion)))
                      case (Some(_)   , false) => logger.info(s"${prefix(payload)} not deleting DERIVED-deployed-dependencies - deployed in ${deployedIn.map(_.asString).mkString(",")} environments")
                                                  EitherT.right[String](Future.unit)
                      case __                  => EitherT.right[String](Future.unit)
                    }
      ...
      _          <- if (isDeployment && deps.isEmpty) {
                      logger.info(s"${prefix(payload)} adding to DERIVED-deployed-dependencies")
